...
class MDSSaldoScraper:
    """
    Direct Selenium scraper for MDS Saldo Detalhado por Conta (Detailed Account Balance).
    Extracts detailed balance data from Brazilian Ministry of Social Development portal.
    """
    
    def __init__(self):
        self.base_url = "https://aplicacoes.mds.gov.br/suaswebcons/restrito/execute.jsf?b=*tbmepQbsdfmbtQbhbtNC&event=*fyjcjs"
        self.download_base_path = Path("downloads/mds_saldo_detalhado")
        self.driver = None
    ...
    def execute_scraping(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            config: Contains 'year', 'month', 'municipality', and other parameters
        ...
        month = config.get('month', 1)  # Required for this scraper
        municipality = config.get('municipality', 'ALL_MG')
        
        # Create download directory
        download_path = self._prepare_download_path(year, month, municipality)
        ...
        try:
            logger.info(f"Starting MDS Saldo scraping - Year: {year}, Month: {month}, Municipality: {municipality}")
            ...
            if municipality == 'ALL_MG':
                ...
                for i, mun_name in enumerate(municipalities, 1):
                    try:
                        ...
                        mun_data = self._extract_municipality_data(year, month, mun_name)
                        if mun_data:
                ...
                mun_data = self._extract_municipality_data(year, month, municipality)
                if mun_data:
                    extracted_data.extend(mun_data)  
            
            # Save extracted data to files
            if extracted_data:
                downloaded_files = self._save_data_files(extracted_data, download_path, year, month, municipality)
    ...
    def _prepare_download_path(self, year: int, month: int, municipality: str) -> Path:
        ...
        month_path = year_path / f"{month:02d}_{month_names[month]}"
        
        if municipality == 'ALL_MG':
            final_path = month_path / "todos_municipios_mg"
        else:
            # Clean municipality name for filesystem
            clean_name = municipality.replace(' ', '_').replace('/', '_')
            final_path = month_path / clean_name
    ...
    def _extract_municipality_data(self, year: int, month: int, municipality: str) -> List[Dict[str, Any]]:
        """Extract balance data for a specific municipality, year and month"""
        try:
            logger.info(f"Extracting data for {municipality} - {year}/{month:02d}")
            ...
            # Extract table data
            data = self._extract_table_data(municipality, year, month)
        ...
        try:
            ...
            for selector in month_selectors:
                try:
                    ...
                    # Try different month representations
                    month_options = [
                        month_names[month],  # Full name
                        f"{month:02d}",      # Zero-padded number
                        str(month),          # Number
                        f"{month:02d} - {month_names[month]}"  # Combined format
                    ]
                    
                    for month_option in month_options:
                        try:
                            select.select_by_visible_text(month_option)
                            logger.info(f"Selected month: {month_option}")
                            return
                        except:
                            continue
                    ...
                    try:
                        ...
                        logger.info(f"Selected month by value: {month}")
                        return
                    except:
                        pass
            ...
            logger.warning(f"Could not find month dropdown to select {month}")
            
        except Exception as e:
            logger.error(f"Failed to select month {month}: {e}")
    ...
    def _extract_table_data(self, municipality: str, year: int, month: int) -> List[Dict[str, Any]]:
    ...
    def _save_data_files(self, data: List[Dict[str, Any]], download_path: Path, year: int, month: int, municipality: str) -> List[str]:
        ...
        try:
            ...
            if municipality == 'ALL_MG':
                base_name = f"mds_saldo_mg_{year}_{month:02d}"
            else:
                clean_mun = municipality.replace(' ', '_').replace('/', '_')
                base_name = f"mds_saldo_{clean_mun}_{year}_{month:02d}"
